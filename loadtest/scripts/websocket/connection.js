...
export const ws_connection_duration = new Trend("ws_connection_duration");
...
export default function (wsUrl, token, handler, timeout = 30) {
  ...
  try {
      ...
      socket.on("message", function (message) {
        ...
        try {
          ...
          if (data.action) {
            ...
            messageStates[`${data.action}_count`] =
              (messageStates[`${data.action}_count`] || 0) + 1;
            messageStates[`${data.action}_data`] = data.data;
          }

          logWebSocketEvent(
            "message_received",
            `收到訊息 (#${messagesReceived})`,
            {
              action: data.action,
              connection_id: connectionId,
          ...
          switch (data.action) {
            case "room_joined":
              logInfo(
                `✅ 收到 room_joined: ${data.data?.message || "unknown"}`
              );
              break;
            case "room_left":
            ...
            case "error":
              logError(
                `❌ WebSocket 錯誤: ${data.data?.message || "unknown error"}`
              );
              break;
            default:
    ...
    // 記錄最終統計
    logInfo(
      `📊 最終統計 - 收到 ${receivedMessages.length} 條訊息，發送 ${messagesSent} 條訊息`
    );
    const stateKeys = Object.keys(messageStates).filter(
      (k) => !k.endsWith("_count") && !k.endsWith("_data")
    );
