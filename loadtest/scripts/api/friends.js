  ...
  group("API - Friends", function () {
    group("Get Friend List", function () {
      ...
      check(res, {
        ...
        "Get Friend List: response has success status": (r) =>
    ...
    group("Get Pending Requests", function () {
      ...
      check(res, {
        ...
        "Get Pending Requests: response has success status": (r) =>
          r.json("status") === "success",
      });
    });
    ...
    group("Get Blocked Users", function () {
      ...
      check(res, {
        ...
        "Get Blocked Users: response has success status": (r) =>
          r.json("status") === "success",
      });
    });
    ...
    if (targetUsername) {
      group("Send Friend Request", function () {
        ...
        logHttpResponse("POST /friends/requests", res, {
          expectedStatus: [200, 400, 403],
        });

        check(res, {
          "Send Friend Request: status is 200, 400 or 403": (r) =>
            [200, 400, 403].includes(r.status),
          "Send Friend Request: response has status field": () =>
            body && body.status !== undefined,
          "Send Friend Request: logic is correct": () => {
            if (res.status === 200) return body && body.status === "success";
            // 403 通常是 CSRF 或權限問題
            if (res.status === 403) return true;
            // 400 通常是業務邏輯錯誤（如已是好友）
            if (res.status === 400) return body && body.status === "error";
            return false;
          },
        });
    ...
    if (friendId) {
      group("Update Friend Status", function () {
        ...
        logHttpResponse("PUT /friends/requests/{id}/accept", res, {
          expectedStatus: [200, 400, 403],
        });

        check(res, {
          "Update Friend Status: status is 200, 400 or 403": (r) =>
            r.status === 200 || r.status === 400 || r.status === 403,
          "Update Friend Status: response has status field": (r) =>
            r.json("status") !== undefined,
        });
